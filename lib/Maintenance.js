...
function serverMaintenance(server, serverID){
	...
	// Added vote number tracker
		if (!server.static.voteTracker.number) server.static.voteTracker.number = 0;
		for (let vote in server.static.voteTracker.votes){
			if (!server.static.voteTracker.votes[vote].number) server.static.voteTracker.votes[vote].number = 0;
		}
}
