	...
	/* performLimitChecks SUBFUNCTIONS */
		...
		function userSentMessage(message, tracker, amount){
			...
			if (user.addSent(amount)){
				...
				if (user.warned == true){
					message.author.sendMessage(`You have been softbanned for going over the limit in ${message.channel.name}. Please let the mods know in #time-out if you wish to appeal the ban.`);
					muteUser(message.author.id, message.channel.id, message.guild.id, tracker.type, true);
				}
				user.warned = true;
				message.delete();
			}
	...
	function muteUser(userID, channelID, serverID, command, ban) {
		...
		if (!ban){		
			...
			user.addRole("256713025061519370");
		}
	}
