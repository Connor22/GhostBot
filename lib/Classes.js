		...
		// Getters
			...
			getWarnMessage(tracker, userID){
				return "You've hit the " + fetchTracker(tracker, this).limit + " " + tracker + " limit in `#" + 
					...
					makeReadable(this.getTimeRemaining(fetchTracker(tracker, this), userID) + 2000) + " before posting again";
			};
		...
		//Misc
			update(){
				...
				for (let trackrName in trackrDict){
					...
					for (let user in trackr.users){
						if (this.getTimeRemaining(trackr, user) < 0){
							delete trackr.users[user];
						}
					}
				}		
			}
			...
			addSent(userID, tracker){
				...
				if (tUser){
					...
					tUser = trackr.users[userID];
				}

				if (tUser.sent >= trackr.limit) return true;
			}
