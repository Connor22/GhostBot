...
// Archive
	...
	GhostBot.Archive.Channel = function(){
...
// Discord
	...
	GhostBot.discordChannelSchemaMethods = Object.assign({
		archive : function(message, messageID){
			const channelModel = GhostBot.Archive.Channel();
...
async function archive100next(messageID, channelID){
	...
	const channel = await GhostBot.channels.get(channelID);
	channel.fetchMessages({after: messageID, limit: 100}).then(messages => {
...
function archiveAnnounce(channelID, storageChannel, size){
	...
	if (size != 100) {
		const channel = GhostBot.channels.get(channelID);
